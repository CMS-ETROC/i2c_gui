...
def interactive_load_baselines(db_dir: str, chip_names: list) -> dict:
    """
    Reads the SQLite DB, presents a menu of unique histories,
    ...
    # 1. Fetch unique histories
    with sqlite3.connect(db_path) as conn:
        try:
            # We group by both note and timestamp in case you used the same note twice
            query = "SELECT DISTINCT save_notes, timestamp FROM baselines ORDER BY timestamp DESC"
    ...
    for idx, row in history.iterrows():
        print(f" [{idx}] Note: '{row['save_notes']}' | Date: {row['timestamp']}")
    print("="*40)
    ...
    while True:
        try:
            ...
            if 0 <= choice_idx < len(history):
                ...
                selected_time = history.iloc[choice_idx]['timestamp']
                break
            else:
    ...
    # 4. Fetch the actual data for the active chips
    chip_dfs = {}
    with sqlite3.connect(db_path) as conn:
        for chip in chip_names:
            query = f"SELECT * FROM baselines WHERE save_notes='{selected_note}' AND timestamp='{selected_time}' AND chip_name='{chip}'"
            df = pd.read_sql_query(query, conn)

            if df.empty:
                print(f"  WARNING: No data found for chip '{chip}' in this history!")
            else:
